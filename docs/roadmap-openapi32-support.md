...
|---------|:--------:|:---------:|-------|
| Parameter `style` / `explode` serialization | 3.0 | ❌ | No serialization handling exists; parameters are extracted but wire-encoding rules are ignored. **Blocks** the cleanest 3.2 cookie-`style`/`allowReserved` work |
| `allowReserved` on query parameters | 3.0 | ❌ | Pre-existing; 3.2 broadens it to headers and any `in` |
