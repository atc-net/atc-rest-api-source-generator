...
export class ApiClient {
  ...
  async request<T>(method: string, path: string, options?: RequestOptions): Promise<ApiResult<T>> {
    ...
    if (options?.body !== undefined) {
      if (options.body instanceof FormData) {
        ...
        fetchBody = JSON.stringify(options.body, dateReplacer);
      }
    }
  ...
  private async handleResponse<T>(response: Response, responseType?: 'json' | 'blob' | 'text'): Promise<ApiResult<T>> {
    ...
    if (response.ok) {
      const data = isText ? await response.text() : isJson ? JSON.parse(await response.text(), dateReviver) : await response.blob();
      const status = response.status === 201 ? 'created' as const : 'ok' as const;
      return { status, data: data as T, response };
    }

    const errorBody = isJson ? JSON.parse(await response.text(), dateReviver) : null;
