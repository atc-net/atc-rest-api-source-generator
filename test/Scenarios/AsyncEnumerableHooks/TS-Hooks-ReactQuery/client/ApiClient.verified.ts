...
export class ApiClient {
  ...
  private async handleResponse<T>(response: Response, responseType?: 'json' | 'blob' | 'text'): Promise<ApiResult<T>> {
    ...
    if (response.ok) {
      let data: unknown;
      try {
        data = isText ? await response.text() : isJson ? JSON.parse(await response.text(), dateReviver) : await response.blob();
      } catch (parseError) {
    ...
    if (isJson) {
      try {
        errorBody = JSON.parse(await response.text(), dateReviver);
      } catch {
        errorBody = null;
      }
    }
