// <auto-generated />
#nullable enable

using System;
using System.CodeDom.Compiler;
using System.Collections.Generic;
using System.Net;
using Atc.Rest.Client;
using InlineSchemas.Generated;
using InlineSchemas.Generated.Reports.Endpoints.Interfaces;
using InlineSchemas.Generated.Reports.Models;

namespace InlineSchemas.Generated.Reports.Endpoints.Results;

/// <summary>
/// Client Endpoint result.
/// Description: List all reports
/// Operation: ListReports.
/// </summary>
[GeneratedCode("Atc.Rest.Api.SourceGenerator", "1.0.0")]
public partial class ListReportsEndpointResult : EndpointResponse, IListReportsEndpointResult
{
    public ListReportsEndpointResult(EndpointResponse response)
        : base(response)
    {
    }

    /// <summary>
    /// Creates a result representing HTTP OK.
    /// </summary>
    public static ListReportsEndpointResult Ok(IEnumerable<ListReportsResponseItem> content)
        => Create(HttpStatusCode.OK, content);

    /// <summary>
    /// Creates a result representing HTTP BadRequest.
    /// </summary>
    public static ListReportsEndpointResult BadRequest(ProblemDetails content)
        => Create(HttpStatusCode.BadRequest, content);

    /// <summary>
    /// Creates a result representing HTTP InternalServerError.
    /// </summary>
    public static ListReportsEndpointResult InternalServerError(ProblemDetails content)
        => Create(HttpStatusCode.InternalServerError, content);

    /// <summary>
    /// Creates a result representing HTTP GatewayTimeout.
    /// </summary>
    public static ListReportsEndpointResult GatewayTimeout(ProblemDetails content)
        => Create(HttpStatusCode.GatewayTimeout, content);

    private static ListReportsEndpointResult Create(
        HttpStatusCode statusCode,
        object? contentObject)
        => new(new EndpointResponse(
            isSuccess: (int)statusCode is >= 200 and < 300,
            statusCode: statusCode,
            content: string.Empty,
            contentObject: contentObject,
            headers: new Dictionary<string, IEnumerable<string>>(StringComparer.Ordinal)));

    public bool IsOk
        => StatusCode == HttpStatusCode.OK;

    public bool IsBadRequest
        => StatusCode == HttpStatusCode.BadRequest;

    public bool IsInternalServerError
        => StatusCode == HttpStatusCode.InternalServerError;

    public bool IsGatewayTimeout
        => StatusCode == HttpStatusCode.GatewayTimeout;

    public IEnumerable<ListReportsResponseItem> OkContent
        => IsOk && ContentObject is IEnumerable<ListReportsResponseItem> result
            ? result
            : throw InvalidContentAccessException<IEnumerable<ListReportsResponseItem>>(HttpStatusCode.OK, "OkContent");

    public ProblemDetails BadRequestContent
        => IsBadRequest && ContentObject is ProblemDetails result
            ? result
            : IsBadRequest && ContentObject is string message
                ? ProblemDetailsFactory.Create(HttpStatusCode.BadRequest, message)
                : IsBadRequest && ContentObject is null
                    ? ProblemDetailsFactory.Create(HttpStatusCode.BadRequest)
                    : throw InvalidContentAccessException<ProblemDetails>(HttpStatusCode.BadRequest, "BadRequestContent");

    public ProblemDetails InternalServerErrorContent
        => IsInternalServerError && ContentObject is ProblemDetails result
            ? result
            : IsInternalServerError && ContentObject is string message
                ? ProblemDetailsFactory.Create(HttpStatusCode.InternalServerError, message)
                : IsInternalServerError && ContentObject is null
                    ? ProblemDetailsFactory.Create(HttpStatusCode.InternalServerError)
                    : throw InvalidContentAccessException<ProblemDetails>(HttpStatusCode.InternalServerError, "InternalServerErrorContent");

    public ProblemDetails GatewayTimeoutContent
        => IsGatewayTimeout && ContentObject is ProblemDetails result
            ? result
            : IsGatewayTimeout && ContentObject is string message
                ? ProblemDetailsFactory.Create(HttpStatusCode.GatewayTimeout, message)
                : IsGatewayTimeout && ContentObject is null
                    ? ProblemDetailsFactory.Create(HttpStatusCode.GatewayTimeout)
                    : throw InvalidContentAccessException<ProblemDetails>(HttpStatusCode.GatewayTimeout, "GatewayTimeoutContent");
}