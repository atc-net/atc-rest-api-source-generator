...
const eventsSeqKeys = {
  all: ['eventsSeq'] as const,
};

export { eventsSeqKeys };

/** Stream events as a JSON text sequence */
export function useStreamEventsSeqStream(options?: { enabled?: boolean }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        for await (const item of api.eventsSeq.streamEventsSeq(controller.signal)) {
