...
export class ApiClient {
  ...
  /**
   * Toggle strict runtime validation. When enabled, schema-mismatch
   * failures throw an Error with the Zod issues instead of returning a
   * 'schemaMismatch' ApiResult arm. Useful in dev/staging to surface
   * spec drift loudly; leave off in prod to keep errors as values.
   */
  ...
  async request<T>(method: string, path: string, options?: RequestOptions): Promise<ApiResult<T>> {
    ...
    return this.handleResponse<T>(response, options?.responseType, options?.parseSchema);
  }
  ...
  private async handleResponse<T>(response: Response, responseType?: 'json' | 'blob' | 'text', parseSchema?: ZodTypeAny): Promise<ApiResult<T>> {
    ...
    if (response.ok) {
      ...
      if (parseSchema && isJson) {
        const parsed = parseSchema.safeParse(data);
        if (!parsed.success) {
          if (this.strictMode) {
            const issuesSummary = parsed.error.issues.map(i => `${i.path.join('.')}: ${i.message}`).join('; ');
            throw new Error(`Schema mismatch: ${issuesSummary}`);
          }

          return { status: 'schemaMismatch', issues: parsed.error.issues, data, response };
        }
