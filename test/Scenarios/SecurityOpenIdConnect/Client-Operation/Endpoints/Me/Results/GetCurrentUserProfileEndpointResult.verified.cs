// <auto-generated />
#nullable enable

using System;
using System.CodeDom.Compiler;
using System.Collections.Generic;
using System.Net;
using Atc.Rest.Client;
using SecurityOpenIdConnect.Generated;
using SecurityOpenIdConnect.Generated.Me.Endpoints.Interfaces;
using SecurityOpenIdConnect.Generated.Me.Models;

namespace SecurityOpenIdConnect.Generated.Me.Endpoints.Results;

/// <summary>
/// Client Endpoint result.
/// Description: Get detailed user profile
/// Operation: GetCurrentUserProfile.
/// </summary>
[GeneratedCode("Atc.Rest.Api.SourceGenerator", "1.0.0")]
public sealed class GetCurrentUserProfileEndpointResult : EndpointResponse, IGetCurrentUserProfileEndpointResult
{
    public GetCurrentUserProfileEndpointResult(EndpointResponse response)
        : base(response)
    {
    }

    /// <summary>
    /// Creates a result representing HTTP OK.
    /// </summary>
    public static GetCurrentUserProfileEndpointResult Ok(UserProfile content)
        => Create(HttpStatusCode.OK, content);

    /// <summary>
    /// Creates a result representing HTTP Unauthorized.
    /// </summary>
    public static GetCurrentUserProfileEndpointResult Unauthorized(ProblemDetails content)
        => Create(HttpStatusCode.Unauthorized, content);

    /// <summary>
    /// Creates a result representing HTTP Forbidden.
    /// </summary>
    public static GetCurrentUserProfileEndpointResult Forbidden(ProblemDetails content)
        => Create(HttpStatusCode.Forbidden, content);

    /// <summary>
    /// Creates a result representing HTTP InternalServerError.
    /// </summary>
    public static GetCurrentUserProfileEndpointResult InternalServerError(ProblemDetails content)
        => Create(HttpStatusCode.InternalServerError, content);

    /// <summary>
    /// Creates a result representing HTTP GatewayTimeout.
    /// </summary>
    public static GetCurrentUserProfileEndpointResult GatewayTimeout(ProblemDetails content)
        => Create(HttpStatusCode.GatewayTimeout, content);

    private static GetCurrentUserProfileEndpointResult Create(
        HttpStatusCode statusCode,
        object? contentObject)
        => new(new EndpointResponse(
            isSuccess: (int)statusCode is >= 200 and < 300,
            statusCode: statusCode,
            content: string.Empty,
            contentObject: contentObject,
            headers: new Dictionary<string, IEnumerable<string>>(StringComparer.Ordinal)));

    public bool IsOk
        => StatusCode == HttpStatusCode.OK;

    public bool IsUnauthorized
        => StatusCode == HttpStatusCode.Unauthorized;

    public bool IsForbidden
        => StatusCode == HttpStatusCode.Forbidden;

    public bool IsInternalServerError
        => StatusCode == HttpStatusCode.InternalServerError;

    public bool IsGatewayTimeout
        => StatusCode == HttpStatusCode.GatewayTimeout;

    public UserProfile OkContent
        => IsOk && ContentObject is UserProfile result
            ? result
            : throw InvalidContentAccessException<UserProfile>(HttpStatusCode.OK, "OkContent");

    public ProblemDetails UnauthorizedContent
        => IsUnauthorized && ContentObject is ProblemDetails result
            ? result
            : IsUnauthorized && ContentObject is string message
                ? ProblemDetailsFactory.Create(HttpStatusCode.Unauthorized, message)
                : IsUnauthorized && ContentObject is null
                    ? ProblemDetailsFactory.Create(HttpStatusCode.Unauthorized)
                    : throw InvalidContentAccessException<ProblemDetails>(HttpStatusCode.Unauthorized, "UnauthorizedContent");

    public ProblemDetails ForbiddenContent
        => IsForbidden && ContentObject is ProblemDetails result
            ? result
            : IsForbidden && ContentObject is string message
                ? ProblemDetailsFactory.Create(HttpStatusCode.Forbidden, message)
                : IsForbidden && ContentObject is null
                    ? ProblemDetailsFactory.Create(HttpStatusCode.Forbidden)
                    : throw InvalidContentAccessException<ProblemDetails>(HttpStatusCode.Forbidden, "ForbiddenContent");

    public ProblemDetails InternalServerErrorContent
        => IsInternalServerError && ContentObject is ProblemDetails result
            ? result
            : IsInternalServerError && ContentObject is string message
                ? ProblemDetailsFactory.Create(HttpStatusCode.InternalServerError, message)
                : IsInternalServerError && ContentObject is null
                    ? ProblemDetailsFactory.Create(HttpStatusCode.InternalServerError)
                    : throw InvalidContentAccessException<ProblemDetails>(HttpStatusCode.InternalServerError, "InternalServerErrorContent");

    public ProblemDetails GatewayTimeoutContent
        => IsGatewayTimeout && ContentObject is ProblemDetails result
            ? result
            : IsGatewayTimeout && ContentObject is string message
                ? ProblemDetailsFactory.Create(HttpStatusCode.GatewayTimeout, message)
                : IsGatewayTimeout && ContentObject is null
                    ? ProblemDetailsFactory.Create(HttpStatusCode.GatewayTimeout)
                    : throw InvalidContentAccessException<ProblemDetails>(HttpStatusCode.GatewayTimeout, "GatewayTimeoutContent");
}