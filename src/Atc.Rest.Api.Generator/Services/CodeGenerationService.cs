...
public static class CodeGenerationService
{
    ...
    /// <summary>
    /// Generates the ParsableList&lt;T&gt; helper type if the OpenAPI document has array query parameters.
    /// </summary>
    /// <param name="openApiDoc">The OpenAPI document.</param>
    /// <param name="projectName">The project name for namespace.</param>
    /// <returns>Generated ParsableList type, or null if no array query parameters exist.</returns>
    public static GeneratedType? GenerateParsableList(
        OpenApiDocument openApiDoc,
        string projectName)
    {
        if (!HasQueryArrayParameters(openApiDoc))
        {
            return null;
        }
        ...
        builder.AppendLine("    where T : IParsable<T>");
        builder.AppendLine("{");
        builder.AppendLine("    /// <inheritdoc />");
        builder.AppendLine("    public static ParsableList<T> Parse(string s, IFormatProvider? provider)");
        builder.AppendLine("    {");
        builder.AppendLine("        var list = new ParsableList<T>();");
        builder.AppendLine("        if (!string.IsNullOrEmpty(s))");
        builder.AppendLine("        {");
        builder.AppendLine("            foreach (var item in s.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))");
        builder.AppendLine("            {");
        builder.AppendLine("                list.Add(T.Parse(item, provider));");
        builder.AppendLine("            }");
        builder.AppendLine("        }");
        builder.AppendLine();
        ...
        builder.AppendLine();
        builder.AppendLine("    /// <inheritdoc />");
        builder.AppendLine("    public static bool TryParse(string? s, IFormatProvider? provider, out ParsableList<T> result)");
        builder.AppendLine("    {");
        builder.AppendLine("        result = new ParsableList<T>();");
        builder.AppendLine("        if (string.IsNullOrEmpty(s))");
        builder.AppendLine("        {");
        ...
        builder.AppendLine("        foreach (var item in s.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))");
        builder.AppendLine("        {");
        builder.AppendLine("            if (!T.TryParse(item, provider, out var parsed))");
        builder.AppendLine("            {");
        builder.AppendLine("                return false;");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            result.Add(parsed);");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        return true;");
        builder.AppendLine("    }");
        ...
        return new GeneratedType(
            TypeName: "ParsableList",
            Category: "Utilities",
            Namespace: $"{projectName}.Generated",
            Content: builder.ToString(),
            RequiredUsings: ["System", "System.CodeDom.Compiler", "System.Collections.Generic"],
            GroupName: null,
            SubFolder: "Utilities");
    }
