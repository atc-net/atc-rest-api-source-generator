...
This project is the .NET Aspire v13 AppHost that orchestrates the PetStoreFull distributed application.

## Architecture

The AppHost manages two projects:
- **api**: The PetStoreFull.Api web API project
- **client**: The PetStoreFull.ClientApp console application
...
```csharp
var api = builder.AddProject<Projects.PetStoreFull_Api>("api")
    .WithExternalHttpEndpoints();

var client = builder.AddProject<Projects.PetStoreFull_ClientApp>("client")
    .WithReference(api);
```
...
```bash
dotnet run --project sample/PetStoreFull/PetStoreFull.Aspire
```
...
- **New SDK format**: `<Project Sdk="Aspire.AppHost.Sdk/13.0.0">`
