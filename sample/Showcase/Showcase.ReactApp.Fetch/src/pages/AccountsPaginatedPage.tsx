...
export default function AccountsPaginatedPage() {
  ...
  // PaginatedResult<Accounts>.results is Accounts[] (Account[][]) per spec — flatten to Account[].
  const results = Array.isArray(data?.results) ? data.results.flat() : [];
          ...
          <TableBody>
            {results.map((item, idx) => (
              <TableRow key={String(item.id ?? idx)}>
                ...
                <TableCell>{String(item.tag ?? '')}</TableCell>
              </TableRow>
            ))}
